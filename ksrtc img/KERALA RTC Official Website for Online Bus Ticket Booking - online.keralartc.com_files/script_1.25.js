...
function calendarPopupDob(id){
	
	var currDate=new Date();
	var toDate=new Date();
	
	var date=parseInt(serverdate.getDate());
	var month = serverdate.getMonth() + 1;
    var year = serverdate.getFullYear();
	 
	if(self.gfPop) {
		gfPop.fPopCalendar(document.getElementById(id),[[year-100,month,date],[year,month,date]]);
...
function printPageByDiv(printDivId) {
    ...
    if(document.getElementById("contextPath") != null) {
    	...
    	var cssLink = '<link id="skinCssID" href="' + contextPath + '/_assets/skin/skin_2_44.css" rel="stylesheet" type="text/css" media="all" />';
...
function setSelectedSeatDetail(requestType, seatNumber, seatType, seatIndx) {
	...
	if(seatNumber.indexOf('WL') != -1 && tdId.className.indexOf('avail') != -1) {
...
function validateBookingForm(requestType) {
	var size = parseInt(document.getElementById("currSeatIndex" + requestType).value);
...
function validateBookingSearch(formObj, path) {
	...
	// for offline time table modify search
...
function submitTrack(formObj,path) {
	...
	if(txtObj.value == "" || txtObj.value == "0") {
		alert("Please enter Txn. Password Number");
		txtObj.focus();
		return false;
	}
...
function setAccomodationDetails(actName, id,accomType) {
	...
	for ( var i = 1; i <= size; i++) {
		...
		if(seat != null){
			if(seat.classList.contains('selectedSeatClassB')){
				seats = seats + 1;
			}
		}
...
function submitUserForm(formObj){
	...
	if(document.getElementById('mobileNo') && document.getElementById('mobileNo').value != '') {
	...
	for(var i = 0; i < 10 ; i++) {
		if(document.getElementById("categoryCodeId" + rt + i) != null
			...
			if(chk == true) {
				...
				if(document.getElementById("passportNo" + oj +indx)!=null){
				document.getElementById("dob" + rt + i).value = document.getElementById("dob" + oj +i).value;
				document.getElementById("passportNo" + rt + i).value = document.getElementById("passportNo" + oj +i).value;
				document.getElementById("foreignerAddress" + rt + i).value = document.getElementById("foreignerAddress" + oj +i).value;
				document.getElementById("nationality" + rt + i).value = document.getElementById("nationality" + oj +i).value;
				}
...
function confirmCancellation(path, formObj) {
	...
	if(seats.length == 0) {
		alert("Atleast one seat must be selected for cancellation.");
		return false;
	} 
...
function validateNameReset(txt, name) {
	setHelpText(txt, name);
	copyPrimaryPaxName();
}

function validateMobileReset(txt, name) {
	setHelpText(txt, name);
	var obj = document.getElementById(name);
	if(!validateMobileNo(obj)) {
		return false;
	}
}

function validateEmailReset(txt, name) {
	setHelpText(txt, name);
	var obj = document.getElementById(name);
...
function addNewPaxRow(act, idx, divId, tdId, seatIndx, seatType) {
	...
	_rEle = '<tbody  class="paxHeadRowCS paxDetailsRow" id="' + nStr + "tr" + '"><tr>'
	+ '<td align="center" class="leftInfoFormLabel">'
	+'Seats</td><td>'
		+ '<input type="text" name="seatDetails" id="seatDetails' + nStr + '" size="1" class="seatNormalField" readonly="readonly">'
	+ '</td></tr><tr><td align="center" class="leftInfoFormLabel" width= "15%">'
	+'Gender'
	+'</td><td  width= "40%">' 
				...
				+ '</select>'
			+ '</td></tr><tr><td align="center" class="leftInfoFormLabel">'
			+'Name'
			+'</td><td>'
			...
			+ '</td></tr><tr>'
			+ '<td align="center" class="leftInfoFormLabel">Nationality'
			+'</td><td>'
			+ '<select name="nationality" id="nationality'+nStr+'" onchange="enabledPassPortNo(\'' + nStr + '\')" class="requiredfield" style="width: 150px;opacity: 0.5;pointer-events: none;cursor: default;">'
			+countriesListOptions+
			+'</select>'
			+ '</td></tr><tr class="hiddenForeignerId hiddenForeigner hiddenSpecific'+nStr+'">'	
			+ '<td align="center" class="leftInfoFormLabel">Passport or Travel Document</td><td>'
			+ '<input type="text" name="passportNo" id="passportNo'+nStr+'" maxlength="20"  size="15" readonly="readonly" class="requiredfield"/>'						
			+ '</td></tr><tr  class="hiddenForeignerId hiddenForeigner hiddenSpecific'+nStr+'">'	
			+ '<td align="center" class="leftInfoFormLabel">Address</td><td>'
			+ '<textarea name="foreignerAddress" id="foreignerAddress'+nStr+'"  maxlength="100" rows="3" cols="30"  readonly="readonly" wrap="hard" class="requiredfield"/>'
			+'</td></tr><tr  class="hiddenForeignerId hiddenForeigner hiddenSpecific'+nStr+'">'						
			+ '<td align="center" class="leftInfoFormLabel">DOB</td><td align="left" valign="top" width="15%" >'						
		...
		;
			...
			+'</td><td  width= "40%">' 
					...
					+ '<td align="center" class="leftInfoFormLabel">Nationality'
					+'</td><td>'
					+ '<select name="nationality" id="nationality'+nStr+'" onchange="enabledPassPortNo(\'' + nStr + '\')" class="requiredfield" style="width: 150px;">'
					+countriesListOptions+
					+'</select>'
					+ '</td></tr><tr class="hiddenForeignerId hiddenForeigner hiddenSpecific'+nStr+'">'	
					+ '<td align="center" class="leftInfoFormLabel">Passport or Travel Document</td><td>'
...
var countriesListOptions=document.getElementById('nationality').innerHTML;
	...
	_rEle = '<tr id="' + nStr + "tr" + '"><td>' 
			...
			+ '</td>'
			+ '<td>'
			+ '<select name="nationality" id="nationality'+nStr+'" onchange="enabledPassPortNo(\'' + nStr + '\')" class="requiredfield">'
			+countriesListOptions+
			+'</select>'
			+ '</td>'
			+ '<td>'
			+ '<input type="text" name="passportNo" id="passportNo'+nStr+'"  size="15" disabled="disabled" class="requiredfield"/>'						
			+ '</td>'
			+ '<td>'
			+ '<input type="text" name="foreignerAddress" id="foreignerAddress'+nStr+'"  size="16" disabled="disabled" class="requiredfield"/>'
			+'</td>'						
			+ '<td align="left" valign="top" width="15%" >'						
			+ '<input type="text" name="dob"  id="dob'+nStr+'"	class="calOnward requiredfield" id="dob'+nStr+'" size="11" maxlength="10" value="" disabled="disabled"   /> '
			+'</td>'
			+ '</tr>'
		;
...
function enabledPassPortNo(objTxt){
	if(document.getElementById('nationality'+objTxt).value!='IN'){
		...
		var hiddenForeignerId=document.getElementsByClassName('hiddenSpecific'+objTxt);
		
		for(var k=0;k<hiddenForeignerId.length;k++){
			hiddenForeignerId[k].classList.remove("hiddenForeignerId");
		}
		
		document.getElementById('passportNo'+objTxt).classList.remove("hiddenForeignerId");
		document.getElementById('foreignerAddress'+objTxt).classList.remove("hiddenForeignerId");
		document.getElementById('dob'+objTxt).classList.remove("hiddenForeignerId");
...
function setPgwSelectedPg(v) {
	document.getElementById("pgId").value = v;
	var retTotalAmt=0, retBankTxnAmt=0,retHdfcBankTxnAmt=0,retPhonePeTxnAmt=0;
	...
	if(document.getElementById("retBankTxnAmountHid")!= null){
		retBankTxnAmt = parseFloat(document.getElementById("retBankTxnAmountHid").value);
		 ...
		 retPhonePeTxnAmt = parseFloat(document.getElementById("retPhonePeBankTxnAmountHid").value);
	...
	if(document.getElementById("pgId").value == '2'){
		...
		document.getElementById("grandTotAmtTd").innerHTML = totalAmt - bankTxnAmt + hdfcBankTxnAmt ;
		...
		if(document.getElementById("retBankTxnAmountHid")!= null){
			...
			document.getElementById("grandTotAmtTd").innerHTML = totalAmt - bankTxnAmt + hdfcBankTxnAmt + retTotalAmt - retBankTxnAmt + retHdfcBankTxnAmt ;
			if(document.getElementById("grandTotalAmtTd") != null)
		...
		document.getElementById("grandTotAmtTd").innerHTML = totalAmt - bankTxnAmt + phonePeBankTxnAmt ;
		document.getElementById("bankTxnAmountTd").innerHTML = phonePeBankTxnAmt ;
		if(document.getElementById("retBankTxnAmountHid")!= null){
			document.getElementById("returnTotalAmtTd").innerHTML = retTotalAmt - retBankTxnAmt + retPhonePeTxnAmt ;
			document.getElementById("returnBankTxnAmountTd").innerHTML = retPhonePeTxnAmt ;
			document.getElementById("grandTotAmtTd").innerHTML = totalAmt - bankTxnAmt + phonePeBankTxnAmt + retTotalAmt - retBankTxnAmt + retPhonePeTxnAmt ;
			if(document.getElementById("grandTotalAmtTd") != null)
			document.getElementById("grandTotalAmtTd").innerHTML = totalAmt - bankTxnAmt + phonePeBankTxnAmt + retTotalAmt - retBankTxnAmt + retPhonePeTxnAmt ;
		}
	}else{
		...
		document.getElementById("grandTotAmtTd").innerHTML = totalAmt + retTotalAmt;
		if(document.getElementById("grandTotalAmtTd") != null)
...
function continueReturnBkg() {
	...
	alert("Now, please select the return journey service.");
	 ...
	 hidediv('ForwardAvailableServicesDiv');
	showdiv('ReturnAvailableServicesDiv');
	document.getElementById("retInfoLeftId").focus();
	var uc = 'unSelectedDivCs';
...
function getTranxDetails(formObj, fmId, path) {
	var isTranxSuccess;
	if(document.getElementById("isTranxSuccess_1").checked == true){
		isTranxSuccess = 1;
	}else {
		isTranxSuccess = 0;
	}
	if(document.getElementById('bankRefNo') && document.getElementById('bankRefNo').value != '') {
		if(!validateOBREF(document.getElementById('bankRefNo').value)) {
			alert("Please enter valid OB Ref No.");
			return false;
		}
	}
...
function calculateTotalFare(requestType) {
	...
	totFare = (basic + srt + toll + levies) ;
	bTxn = parseFloat(Math.ceil(parseFloat((totFare * bTxn ) / 100.00)));
...
function validateSeniorSitizenCon() {
	...
	if (obj != null && obj.value != '') {
		...
		if (concessionId == 1468092614493) {
			alert("Senior Citizen concession allowed for residents of Kerala State only.  ID proof Passport/ Voter ID/ Aadhaar/ Driving License/ Identity Card of PSUs of GOI/ NREGA card/ Identity Card of PSUs of GOK/ Identity card  of Senior Citizens Welfare Directorate/ Senior citizen Identity Card issued by KERALA RTC produce while travelling.");
			return true;
		} 
	}
}
...
function loadMore(url){
	try{
		if ((referenceWin == null) || (referenceWin.closed == true)){
			referenceWin = window.open(url, '', 'toolbar=false,statusbar=false,scrollbars=1,addressbar=false,resize=false,width=100%,height=100%,left=10,top=10');
			referenceWin.focus();
		}else{
			referenceWin.focus();
		}
...
function validateFinalTransferMoney(divId){
	var obj;
	var mobileNo;
	obj = document.getElementById("mobileNo");
	if(obj != null && obj.value != ''){
		mobileNo = obj.value;
	}
	var email;
	...
	var generateOTP;
	obj = document.getElementById("generateOTP");
	if(obj != null && obj.value != ''){
		generateOTP = obj.value;
	}
	
	var path =  "/wallet/transferMoney/conform.do?mobileNo=" + mobileNo 
				+ "&email="+ email
	...
	document.getElementById("walletTranserAmount").value = '';
